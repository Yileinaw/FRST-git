...
// --- Move food data definition outside the function ---
const allDefinedFoods: FoodInfo[] = [
...
// --- End of moved food data definition ---
...
// Initialize all food data from the pre-defined array
const initializeFoodData = () => {
  loading.value = true;
  // const foods: FoodInfo[] = [ ... ]; // Remove internal definition
  allFoods.value = [...allDefinedFoods]; // Assign from the top-level constant

  // 动态生成可用分类列表，并确保使用 baseCategories 的中文名
  // Use allDefinedFoods here as well
  const categoriesInFoods = new Set(allDefinedFoods.map(food => food.category).filter(Boolean));
...
.filter-section {
  ...
  .filter-label {
    ...
    margin-bottom: 8px; // Add space below label on small screens
  }

  .el-radio-group.category-filter {
    margin-bottom: 10px; // Add space below categories on small screens
    ...
    .el-radio-button:first-child .el-radio-button__inner {
      border-left: 1px solid var(--el-border-color); // Ensure left border for first item
    ...
    .el-radio-button__original-radio:checked+.el-radio-button__inner {
      ...
      box-shadow: -1px 0 0 0 var(--el-color-primary); // Keep focus style minimal
    }
  }
  ...
  @media (min-width: 768px) {
    .filter-label {
      margin-bottom: 0; // Reset space below label
    }
    .el-radio-group.category-filter {
      margin-bottom: 0; // Reset space below categories
      .el-radio-button {
        margin-bottom: 0; // Reset space between wrapped buttons
      }
    }
    .el-col {
      display: flex;
...
.grid-container-box {
  min-height: 400px; // Ensure minimum height during loading
}
